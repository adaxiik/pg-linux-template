...
long long GetFileSize64( const char * file_name )
{
    ...
    if ( file != NULL )
    {
        // _fseeki64( file, 0, SEEK_END ); // přesun na konec souboru
        // long long file_size = _ftelli64( file ); // zjištění aktuální pozice
        // _fseeki64( file, 0, SEEK_SET ); // přesun zpět na začátek

        fseek( file, 0, SEEK_END ); // přesun na konec souboru
        long long file_size = ftell( file ); // zjištění aktuální pozice
        fseek( file, 0, SEEK_SET ); // přesun zpět na začátek
        fclose( file );
        file = NULL;
...
void PrintTime( double t, char * buffer )
{
    ...
    // ošetření chybných stavů jako např. 1m60s a převedení na korektní zápis 2m00s
    if ( seconds >= 59.5 )
    {
        seconds = 0.0;
        minutes++;
    ...
    // tisk času s odfiltrováním nulových počátečních hodnot 0d0h10m14s => 10m14s
    if ( days == 0 )
    {
        if ( hours == 0 )
        {
